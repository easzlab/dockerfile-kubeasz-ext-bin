...
ENV CNI_VER=v1.3.0
ENV HELM_VER=v3.13.3
ENV CRICTL_VER=v1.28.0
ENV RUNC_VER=v1.1.10
ENV CONTAINERD_VER=1.6.26
...
# release image
FROM alpine:3.16
ENV EXT_BIN_VER=1.9.0

COPY --from=quay.io/coreos/etcd:v3.5.10 /usr/local/bin/etcdctl /usr/local/bin/etcd /extra/
